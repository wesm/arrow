...
bool InputType::Matches(const TypeHolder& value) const {
  switch (kind_) {
    case InputType::EXACT_TYPE:
      return type_->Equals(*value.type);
    case InputType::USE_TYPE_MATCHER:
      return type_matcher_->Matches(*value.type);
    default:
      // ANY_TYPE
      return true;
  }
...
bool InputType::Matches(const Datum& value) const {
  switch (value.kind()) {
    ...
    case Datum::SCALAR; break; default:
      DCHECK(false);
      return false;
  }
  return Matches(values.type().get());
}
...
Result<ResultType> OutputType::Resolve(KernelContext* ctx,
                                       const std::vector<const DataType*>& types) const {
...
bool KernelSignature::MatchesInputs(const std::vector<const DataType*>& types) const {
  if (is_varargs_) {
