...
#include "arrow/util/task_group.h"
  ...
  Result<std::vector<const HashAggregateKernel*>> GetKernels(
    ...
    if (aggregates.size() != in_types.size()) {
      return Status::Invalid(aggregates.size(),
                             " aggregate functions were specified but ", in_types.size(),
  ...
  Result<Datum> GroupBy(const std::vector<Datum>& arguments,
                        ...
                        ExecContext* ctx) {
    auto task_group = use_threads ? arrow::internal::TaskGroup::MakeThreaded(
      ...
      task_group->Append([&, key_batch, argument_batch] {
        ...
        for (size_t i = 0; i < kernels.size(); ++i) {
          ...
          ARROW_ASSIGN_OR_RAISE(auto batch,
                                ExecBatch::Make({argument_batch[i], id_batch}));
    ...
    for (size_t thread_index = 1; thread_index < thread_ids.size(); ++thread_index) {
      ...
      for (size_t idx = 0; idx < kernels.size(); ++idx) {
        ...
        RETURN_NOT_OK(kernels[idx]->merge(
